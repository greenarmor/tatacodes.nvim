...
describe('tatacodes.nvim multi-installer matrix flow', function()
  before_each(function()
    vim.cmd 'set noswapfile'
    vim.cmd 'silent! bwipeout!'

    -- Capture notify messages for assertions
    _G.__tatacodes_notify_log = {}
    vim.notify = function(msg, level)
      table.insert(_G.__tatacodes_notify_log, { msg = msg, level = level })
      print('[notify]', msg)
    end
  ...
  it('tries each supported PM and handles success/failure gracefully', function()
    local installer = require 'tatacodes.installer'
    local state = require 'tatacodes.state'
    ...
    for _, pm in ipairs(available) do
      ...
      installer.run_install(pm, function()
        triggered = true
        local win = state.win
        assert(win and vim.api.nvim_win_is_valid(win), 'Tatacodes float should open on success')
      ...
      for _, entry in ipairs(_G.__tatacodes_notify_log) do
