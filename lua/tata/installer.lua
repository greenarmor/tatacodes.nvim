...
local fallback_instructions = {
  pnpm = [[
    pnpm installed successfully, but Tata Coding Agent may not be in your PATH.
    Run `pnpm setup`, then restart your shell or add the output of `pnpm bin -g` to your PATH.
  ]],
  yarn = [[
    Yarn installed Tata Coding Agent, but the executable may not be in your PATH.
  ...
  bun = [[
    Bun installed Tata Coding Agent, but it may not be available in your PATH.
  ...
  deno = [[
    Deno installed Tata Coding Agent, but it may not be in your PATH.
...
function M.open_install_float()
  ...
  vim.api.nvim_buf_set_option(state.buf, 'filetype', 'tatacodes')
...
function M.run_install(pm, on_success)
  ...
  if not cmd_str then
    vim.notify('[tatacodes.nvim] Unsupported package manager: ' .. pm, vim.log.levels.ERROR)
    return
  end
  ...
  state.job = vim.fn.termopen(cmd_str, {
    ...
    on_exit = function(_, code)
      if code == 0 then
        vim.notify('[tatacodes.nvim] Tata Coding Agent installed successfully via ' .. pm, vim.log.levels.INFO)
        if not M.__test_ignore_path_check and vim.fn.executable 'tata' == 0 then
          local fallback = fallback_instructions[pm]
          if fallback then
            vim.notify('[tatacodes.nvim] Tata Coding Agent not yet available on PATH.\n' .. fallback, vim.log.levels.WARN)
          else
            vim.notify('[tatacodes.nvim] Tata Coding Agent not found on PATH after install. Try restarting your shell.', vim.log.levels.WARN)
          end
        end
        if on_success then
        ...
        if not M.__test_ignore_path_check then
          vim.notify('[tatacodes.nvim] Installation failed via ' .. pm, vim.log.levels.ERROR)
